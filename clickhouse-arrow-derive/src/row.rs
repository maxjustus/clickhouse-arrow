...
pub fn expand_derive_serialize(
    ...
    let impl_block = quote! {
        ...
        use ::clickhouse_arrow::{ToSql as _, FromSql as _};
        #[automatically_derived]
        #[allow(clippy)]
        #[allow(non_snake_case)]
        #[allow(clippy::absurd_extreme_comparisons)]
        impl #impl_generics ::clickhouse_arrow::Row for #ident #ty_generics #where_clause {
            ...
            fn to_schema() -> Option<Vec<(String, ::clickhouse_arrow::Type, Option<::clickhouse_arrow::Value>)>> {
                #schema_body
            }

            fn deserialize_row(map: Vec<(&str, &::clickhouse_arrow::Type, ::clickhouse_arrow::Value)>) -> ::clickhouse_arrow::Result<Self> {
                #deserialize_body
            }

            fn serialize_row(self, type_hints: &[(String, ::clickhouse_arrow::Type)]) -> ::clickhouse_arrow::Result<Vec<(::std::borrow::Cow<'static, str>, ::clickhouse_arrow::Value)>> {
                #serialize_body
            }
        }
    };
...
fn serialize_length_body(cont: &Container, _params: &Parameters) -> Fragment {
    if let Some(_type_into) = cont.attrs.type_into() {
            ...
            .filter(|&field| !field.attrs.skip_serializing() && field.attrs.nested())
        {
            ...
            total = quote! { match <#field_ty as ::clickhouse_arrow::Row>::COLUMN_COUNT { Some(x) => (#total) + x, None => return None, } };
        }
        ...
        let name_sources = cont.data.iter().filter(|&field| !field.attrs.skip_serializing())
            .map(|field| {
                ...
                if field.attrs.nested() {
                    let field_ty = unwrap_vec_type(field.ty).expect("invalid non-Vec nested type");
                    quote! { out.extend(<#field_ty as ::clickhouse_arrow::Row>::column_names()?.into_iter().map(|x| ::std::borrow::Cow::Owned(format!("{}.{}", #name, x)))); }
...
fn serialize_into(params: &Parameters, type_into: &syn::Type) -> Fragment {
    ...
    quote_block! {
        ::clickhouse_arrow::Row::serialize_row(
            ...
            match field.attrs.serialize_with() {
                ...
                None => {
                    if field.attrs.nested() {
                        ...
                        quote! {
                            {
                                let inner_length = <#field_ty as ::clickhouse_arrow::Row>::COLUMN_COUNT.expect("nested structure must have known length");
                                let mut outputs: ::std::vec::Vec<(::std::option::Option<::std::borrow::Cow<str>>, ::std::vec::Vec<::clickhouse_arrow::Value>)> = ::std::vec::Vec::with_capacity(inner_length);
                                for _ in 0..inner_length {
                                    outputs.push((None, ::std::vec::Vec::new()));
                                }
                                for row in #field_expr.into_iter() {
                                    let columns = <#field_ty as ::clickhouse_arrow::Row>::serialize_row(row, type_hints)?;
                                    ...
                                    let name = match name {
                                        ...
                                        None => {
                                            if column_names.is_none() {
                                                column_names = Some(<#field_ty as ::clickhouse_arrow::Row>::column_names().expect("column_names required for empty nested serialization"));
                                            }
                                            format!("{}.{}", #key_expr, column_names.as_ref().unwrap().get(i).expect("missing column_name for nested struct")).into()
                                        }
                                    };
                                    out.push((name, ::clickhouse_arrow::Value::Array(values)));
                                }
                            }
                        }
                    } else if field.attrs.flatten() {
                        ...
                        quote! {
                            ...
                            out.push((::std::borrow::Cow::Borrowed(#key_expr), <#field_ty as ::clickhouse_arrow::ToSql>::to_sql(#field_expr, type_hint)?));
                        }
                    }
                },
            }
...
fn deserialize_from(type_from: &syn::Type) -> Fragment {
    quote_block! {
        ::clickhouse_arrow::Result::map(
            <#type_from as ::clickhouse_arrow::Row>::deserialize_row(map),
...
fn deserialize_try_from(type_try_from: &syn::Type) -> Fragment {
    quote_block! {
        ::clickhouse_arrow::Result::and_then(
            <#type_try_from as ::clickhouse_arrow::Row>::deserialize_row(map),
...
fn deserialize_map(
        ...
        .for_each(|(field, name)| {
            ...
            if field.attrs.nested() {
                ...
                nested_temp_decls.push(quote_spanned! { span=>
                    let #size_field = <#field_ty as ::clickhouse_arrow::Row>::COLUMN_COUNT.expect("nested structure must have known length");
                    let mut #deser_name_ext: Vec<(&str, &::clickhouse_arrow::Type)> = Vec::with_capacity(#size_field);
                    let mut #deser_name_ext_iter: Vec<::std::vec::IntoIter<::clickhouse_arrow::Value>> = Vec::with_capacity(#size_field);
                ...
                name_match_arms.push(quote_spanned! { span=>
                    full_name if full_name.starts_with(#deser_name_dotted) => {
                        let values = _value.unarray().ok_or_else(|| ::clickhouse_arrow::Error::UnexpectedTypeWithColumn(::std::borrow::Cow::Owned(full_name.to_string()), _type_.clone()))?;
                        if #deser_name_ext.is_empty() {
                            #deser_name_ext_len = values.len();
                        } else if #deser_name_ext_len != values.len() {
                            return ::clickhouse_arrow::Result::Err(::clickhouse_arrow::Error::DeserializeError(format!("invalid length for nested columns, mismatches previous column {}: {} != {}", _name, #deser_name_ext_len, values.len())));
                        }
                        #deser_name_ext.push((full_name.strip_prefix(#deser_name_dotted).unwrap(), _type_.unarray().ok_or_else(|| ::clickhouse_arrow::Error::UnexpectedTypeWithColumn(::std::borrow::Cow::Owned(full_name.to_string()), _type_.clone()))?));
                        #deser_name_ext_iter.push(values.into_iter());
                    }
                });
                index_match_arms.push(quote_spanned! { span=>
                    x if x >= (#local_index) && x < (#current_index) => {
                        let values = _value.unarray().ok_or_else(|| ::clickhouse_arrow::Error::UnexpectedTypeWithColumn(::std::borrow::Cow::Owned(_name.to_string()), _type_.clone()))?;
                        if #deser_name_ext.is_empty() {
                            #deser_name_ext_len = values.len();
                        } else if #deser_name_ext_len != values.len() {
                            return ::clickhouse_arrow::Result::Err(::clickhouse_arrow::Error::DeserializeError(format!("invalid length for nested columns, mismatches previous column {}: {} != {}", _name, #deser_name_ext_len, values.len())));
                        }
                        #deser_name_ext.push((_name, _type_.unarray().ok_or_else(|| ::clickhouse_arrow::Error::UnexpectedTypeWithColumn(::std::borrow::Cow::Owned(_name.to_string()), _type_.clone()))?));
                        #deser_name_ext_iter.push(values.into_iter());
                    }
                });
                nested_rectify.push(quote_spanned! { span=>
                    {
                        ...
                        'outer: loop {
                            ...
                            for (name, type_) in #deser_name_ext.iter() {
                                temp.push((*name, *type_, ::clickhouse_arrow::Value::Null));
                            }
                            ...
                            #name.as_mut().unwrap().push(<#field_ty as ::clickhouse_arrow::Row>::deserialize_row(temp)?);
                        }
                    }
                });
                return;
            ...
            let visit = match field.attrs.deserialize_with() {
                None => {
                    let field_ty = field.ty;
                    let span = field.original.span();
                    quote_spanned!(span=> <#field_ty as ::clickhouse_arrow::FromSql>::from_sql(_type_, _value).map_err(|e| e.with_column_name(#deser_name))?)
                }
                Some(path) => {
                    let span = field.original.span();
            ...
            name_match_arms.push(quote_spanned! { span=>
                #deser_name => {
                    if ::std::option::Option::is_some(&#name) {
                        return ::clickhouse_arrow::Result::Err(::clickhouse_arrow::Error::DuplicateField(#deser_name));
                    }
    ...
    let ignored_arm = if cattrs.deny_unknown_fields() {
        quote! {
            _ => {
                return ::clickhouse_arrow::Result::Err(::clickhouse_arrow::Error::UnknownField(_name));
            }
        }
    } else {
        quote! {
    ...
    let mut pull_flatten: Vec<TokenStream> = vec![quote! {
        let mut map = map;
        let mut map_flattened_fields = std::collections::HashMap::<&str, (&::clickhouse_arrow::Type, ::clickhouse_arrow::Value)>::default();
    }];
    for (f, _) in fields_names.iter() {
        ...
        pull_flatten.push(quote! {
            for c in #ty::column_names()
                    .ok_or_else(|| ::clickhouse_arrow::Error::DeserializeError(#missing_names_error.into()))? {
                let idx = map.iter().enumerate().find(|(_, (c2,_,_))| c2 == &c)
                                    .ok_or(::clickhouse_arrow::Error::MissingField(#missing_col_error))?.0;
    ...
    let result = fields_names.iter().map(|(field, name)| {
        ...
        if field.attrs.skip_deserializing() {
            ...
            quote! {
                ...
                clickhouse_arrow::Row::deserialize_row(map2)? }
            }
        } else {
            quote!(#member: #name)
        }
    ...
    quote_block! {
        ...
        ::clickhouse_arrow::Result::Ok(#result)
    }
}
...
fn expr_is_missing(field: &Field, cattrs: &attr::Container) -> Fragment {
    ...
    let name = field.attrs.name().name();
    let span = field.original.span();
    let func = quote_spanned!(span=> ::clickhouse_arrow::Error::MissingField);
    quote_expr! {
        return ::clickhouse_arrow::Result::Err(#func(#name))
    }
}
