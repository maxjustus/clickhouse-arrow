...
mod tests {
    ...
    #[tokio::test]
    async fn test_json_typed_nullability_defaults_e2e() -> Result<(), Box<dyn std::error::Error>> {
        // Start ClickHouse container (shared across tests)
        ...
        client.execute("SET allow_experimental_object_type = 1", None).await?;
        client.execute("SET allow_suspicious_low_cardinality_types = 1", None).await?;

        // Prepare table
        client.execute("DROP TABLE IF EXISTS e2e_json_defaults", None).await?;
        client
            .execute(
                r#"
        ...
        let mut stream = client.insert("INSERT INTO e2e_json_defaults VALUES", block, None).await?;
        ...
        let mut cnt_stream =
            client.query::<Cnt>("SELECT count() AS count FROM e2e_json_defaults", None).await?;
        let mut total = 0u64;
        ...
        while let Some(r) = rows.next().await {
            let r = r?;
            let v: serde_json::Value = serde_json::from_str(&r.data)?;
            seen.push(v);
        }
        ...
        // Cleanup
        client.execute("DROP TABLE e2e_json_defaults", None).await?;
        Ok(())
    }
}
