...
impl DiscriminatorMap {
    ...
    pub(crate) fn new(variant_types: &[Type]) -> Result<Self> {
        ...
        let mut type_strings: Vec<(String, Type)> = variant_types
            .iter()
...
mod tests {
    ...
    fn test_variant_with_complex_types() {
        ...
        let data = vec![
            ...
            2, 0, 0, 0, 0, 0, 0, 0,  // offset 2 (array has 2 elements)
            1, b'a',  // 'a'
            1, b'b',  // 'b'
            // Date data (1 row) - days since 1970-01-01
            19723u16.to_le_bytes()[0], 19723u16.to_le_bytes()[1],  // 2024-01-01
            // UInt64 data (1 row)
            42, 0, 0, 0, 0, 0, 0, 0,  // 42
        ];
    ...
    fn test_variant_multitype_sorting() {
        ...
        let data = vec![
            ...
            3, 0, 0, 0, 0, 0, 0, 0,  // offset 3 (array has 3 elements)
            1, 2, 3,  // values [1, 2, 3]
            // Date data (1 row)
            100u16.to_le_bytes()[0], 100u16.to_le_bytes()[1],  // day 100
            // DateTime data (1 row)
            1234567890u32.to_le_bytes()[0],
            1234567890u32.to_le_bytes()[1],
            1234567890u32.to_le_bytes()[2],
            1234567890u32.to_le_bytes()[3],  // Unix timestamp
            // String data (1 row)
            5, b'h', b'e', b'l', b'l', b'o',  // 'hello'
            // UInt64 data (1 row)
            231, 3, 0, 0, 0, 0, 0, 0,  // 999 (0x03E7 in little-endian)
        ];
        ...
        match &values[4] {
            Value::Variant(2, inner) => {
                match &**inner {
                    Value::DateTime(dt) => {
                        assert_eq!(dt.1, 1234567890);  // Check timestamp
                    }
