...
/// Read a single primitive value from the reader
pub(crate) async fn read_single_primitive_value<R: ClickHouseRead>(
    type_: &Type,
    reader: &mut R,
) -> Result<Value> {
    Ok(match type_ {
        ...
        _ => {
            return Err(crate::Error::DeserializeError(format!(
                "read_single_primitive_value: unsupported type: {type_:?}"
            )));
        }
    })
}
...
pub(crate) async fn read_with_path<R: ClickHouseRead>(
    ...
    for _ in 0..rows {
        out.push(read_single_primitive_value(type_, reader).await?);
    }
    Ok(out)
}
