...
# /// script
# dependencies = ["scapy", "testcontainers"]
# ///
...
def main():
  ...
  ./scripts/chc-tcp.py 'SELECT 1'
  
  # Compact JSON output
  ./scripts/chc-tcp.py 'SELECT 1' --format compact
  
  # Process streaming JSON with jq
  ./scripts/chc-tcp.py 'SELECT 1' --format compact | jq -c 'select(.direction == "SERVER->CLIENT")'
  
  # Capture to pcap file (defaults to compact format)
  ./scripts/chc-tcp.py 'SELECT 1' --pcap capture.pcap
  
  # Capture to pcap with hex output
  ./scripts/chc-tcp.py 'SELECT 1' --pcap capture.pcap --format hex
  
  # Use testcontainer with specific version
  ./scripts/chc-tcp.py 'SELECT version()' --clickhouse-container-version 25.4
  
  # Verbose mode (show debug output to stderr)
  ./scripts/chc-tcp.py 'SELECT 1' --verbose'''
    )
    ...
    parser.add_argument('--clickhouse-container-version', help='Start ClickHouse testcontainer with specified version (e.g., 25.4)')
    
    args = parser.parse_args()
    
    # Set default format based on whether pcap is specified
    if args.format is None:
        args.format = 'compact' if args.pcap else 'hex'
    
    container = None
    host = args.host
    port = args.port
    
    # Start testcontainer if version specified
    if args.clickhouse_container_version:
        if args.verbose:
            print(f"Starting ClickHouse testcontainer version {args.clickhouse_container_version}...", file=sys.stderr)
        
        container = ClickHouseContainer(f"clickhouse/clickhouse-server:{args.clickhouse_container_version}")
        container.start()
        ...
        if args.verbose:
            print(f"ClickHouse testcontainer started at {host}:{port}", file=sys.stderr)
    ...
    try:
        ...
        if container:
            container.stop()
            if args.verbose:
                print("ClickHouse testcontainer stopped", file=sys.stderr)
